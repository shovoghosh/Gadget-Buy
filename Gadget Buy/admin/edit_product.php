	...
	<div id="search-container">
		...
		<?php
			...
			if (isset($_POST['update'])) {
				...
				if (empty($name) || empty($desc) || empty($mp) || empty($sp) || empty($off) || $shipping == "" || empty($tags) ||empty($quantity) || empty($category)) {
					// show error message
					echo "<div class='alert alert-danger'>Fill in all the fields.</div>";
				}else{
					// store into the database
					$update = $db->Update("product","category_id='$category',name='$name',quantity='$quantity',description='$desc',mp='$mp',sp='$sp',off='$off',shipping='$shipping',tags='$tags'","id='$product_id'");
					if ($update) {
		...
		<form action="<?php echo $_SERVER['PHP_SELF']."?id=".$product_id;?>" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				<div class="row">
					<div class="col-sm-3">
						...
						<input type="text" name="pmp" id="pmp" class="form-control" placeholder="Ex: 45000 (NOTE:no tk)" value="<?php echo $product['mp']; ?>">
					</div>
					<div class="col-sm-3">
						<label for="psp">Selling Price</label>
						<input type="text" name="psp" id="psp" class="form-control" placeholder="Ex: 40000 (NOTE:no tk)" value="<?php echo $product['sp']; ?>">
					</div>
					<div class="col-sm-3">
					...
					<div class="col-sm-3">
						...
						<input type="text" name="pshipping" id="pshipping" class="form-control" placeholder="Ex	: 40 (NOTE:no tk)" value="<?php echo $product['shipping']; ?>">
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="pquantity" name="pquantity" placeholder="Enter your product quantity"  value="<?php echo $product['quantity']; ?>">
			</div>
